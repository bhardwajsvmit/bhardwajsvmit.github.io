...
import { srConfig, stats } from '@config';
...
const Stat = ({ value, prefix, suffix, label, delay }) => {
  ...
  useEffect(() => {
    sr.reveal(revealRef.current, srConfig(delay));
  }, [delay]);
...
const Impact = () => {
  ...
  useEffect(() => {
    sr.reveal(eyebrowRef.current, srConfig());
    sr.reveal(titleRef.current, srConfig(60));
  }, []);
