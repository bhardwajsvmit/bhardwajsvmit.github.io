  ...
  return (
    <section id="contact" className="border-t-2 border-ink bg-accent px-[34px] pt-[110px] pb-10 max-thone:px-5 max-thone:pt-16 max-thone:pb-8 text-ink">
      <div className="mx-auto max-w-[1500px]">
        <div className="mb-[18px] font-mono text-[13px] tracking-[0.16em] uppercase">[ 05 ] Let&rsquo;s talk</div>
        <h2
        ...
        <div className="mt-[70px] flex flex-wrap gap-7 border-t-2 border-ink pt-[22px] font-mono text-[13px] font-medium tracking-[0.04em] uppercase">
          {github && (
            <a
              href={github.url}
              target="_blank"
              rel="nofollow noopener noreferrer"
              className="bg-[linear-gradient(#111110,#111110)] bg-left-bottom bg-no-repeat bg-[length:0_2px] pb-0.5 text-inherit no-underline [transition:background-size_0.28s_cubic-bezier(0.2,0.7,0.2,1),letter-spacing_0.28s_ease] hover:bg-[length:100%_2px] hover:tracking-[0.09em]">
              GitHub ↗
            </a>
          )}
          {linkedin && (
          ...
          <span className="ml-auto opacity-70">© {new Date().getFullYear()} Sumit Bhardwaj</span>
        </div>
      </div>
    </section>
  );
